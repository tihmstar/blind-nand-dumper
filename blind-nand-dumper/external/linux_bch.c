...
#define WARN_ON(a) (a)
#define kmalloc(a,b) calloc(a,1)
#define kzalloc(a,b) calloc(a,1)
 ...
 */
int bch_decode(struct bch_control *bch, uint8_t *data, unsigned int len,
           uint8_t *recv_ecc, const uint8_t *calc_ecc,
           const unsigned int *syn)
{
    ...
    unsigned int *errloc = bch->errloc;
    ...
    if (err > 0) {
        ...
        for (int i=0; i<err; i++) {
            unsigned int epos = errloc[i];
            if (epos >= 8*len){
                epos -= 8*len;
                recv_ecc[epos/8] ^= 1 << (epos % 8);
            }else{
                data[epos/8] ^= 1 << (epos % 8);
            }
        }
    }
                 ...
                 bool swap_bits, int ecc_bytes)
{
    ...
    bch->errloc = kzalloc(sizeof(bch->errloc)*t, GFP_KERNEL);
    bch->m = m;
    bch->t = t;
    bch->n = (1 << m)-1;
